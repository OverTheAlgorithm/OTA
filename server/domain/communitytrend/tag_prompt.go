...
func BuildTagPrompt(in TaggerInput) string {
	...
	b.WriteString(fmt.Sprintf("2. 보수적으로 평가하세요. 같은 논제가 %d건 이상일 때만 태그로 인정합니다.\n", in.MinCount))
	b.WriteString("3. 태그 이름은 정밀해야 합니다. 예: '우파'가 아니라 '우파 지지', '남성'이 아니라 '남성 인권'.\n")
	b.WriteString("4. 글 본문을 인용하거나 재현하지 마세요. 논제(주제)만 추출합니다.\n")
	b.WriteString("5. 확정 밈 목록에 있는 표현이 보이면 meme_matches에 카운트하세요(별칭 포함).\n")
	b.WriteString("6. 기존 태그·일반어·밈목록·금지목록에 없는데 여러 번 반복되는 신조어는 meme_candidates에 넣으세요.\n\n")
	...
	b.WriteString(`{"tags":[{"tag_id":0,"name":"태그명","axis_key":"축키","count":3,"is_new":false,"new_axis_key":""}],`)
	b.WriteString(`"meme_matches":[{"meme_id":1,"name":"밈명","count":2}],`)
	b.WriteString(`"meme_candidates":[{"expression":"신조어","hit_count":2}]}`)
