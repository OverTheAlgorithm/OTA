...
### "top" 카테고리 (필수, 3~5개) — 대화 소재
- **사람들이 실제로 대화하는** 주제만 여기에 배치
- "이거 알아?" 하고 대화를 꺼낼 수 있는 주제: 연예, 논란, SNS 화제, 스포츠 명장면, 바이럴 이슈 등
- rank는 1부터 시작, 가장 대화가 되는 주제가 1번
- 한 가지 큰 사건 안에 여러 포인트가 있으면 **가장 대화가 되는 세부 포인트**를 topic으로 잡아주세요
- ⚠️ **"대화 소재 테스트"**: 직장 동료에게 "야 이거 들었어?" 하고 꺼낼 수 있으면 top. 그냥 "아 그렇구나" 하고 끝나면 top이 아님

### "brief" 카테고리 (선택, 0~2개) — 알아두면 좋은 것
- 중요하지만 대화 소재는 아닌 주제: 자연재해, 추모 행사, 국제 외교, 법 개정 등
- "몰라도 대화에 지장 없지만, 알면 교양 있어 보이는" 정보
- **짧게**: summary는 1문장, details는 빈 배열 []
- buzz_score는 실제 검색량/언급량 기준으로 부여 (대화 가능성이 아님)
- 해당하는 주제가 없으면 0개도 OK (억지로 채우지 않음)

### 어디에 넣어야 할지 판단 기준
| 주제 예시 | top? brief? | 이유 |
|-----------|-------------|------|
| 아이돌 열애설 | top | 모든 직장/학교에서 대화 소재 |
| 예능 방송사고 | top | "어제 그거 봤어?" 대화 유발 |
| 산불 발생 | brief | 중요하지만 대화 소재는 아님 |
| 역사적 유물 귀환 | brief | 교양 정보, 대화로 이어지기 어려움 |
| 주가 폭락/급등 | top (수치가 극단적이면) | "코스피 봤어?" 대화 가능 |
| 법안 통과 | brief | 알아야 하지만 캐주얼 대화 어려움 |

### 그 외 카테고리 (선택, 각 1~3개) — 구독 관심사
