...
// brain_category, buzz_score, and select source URLs. No writing — just analysis.
func BuildClusterPrompt(collectedData string, brainCategories []BrainCategory) string {
...
2. **Categorize**: Assign each topic to one of these categories:
...
3. **Brain Category**: Assign each topic a "brain_category" key from the table below.
...
4. **buzz_score**: Calculate buzz_score (1-100) based on CONCRETE DATA, not gut feeling:
...
5. **Source Selection**: Pick the most relevant article URLs from the collected data for each topic.
...
5. Every topic MUST have at least 1 source. No empty sources arrays.`, dateStr, collectedData, formatBrainCategoryTable(brainCategories))
}
