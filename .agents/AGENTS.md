...
| ct_communities | Community natural keys and descriptors for trend adapters |
| ct_axes | Tag axes (leaning, political, age, etc.) |
| ct_tags | Taxonomy tag pool (attachment to community/daily stat) |
| ct_community_tags | Meta (leaning) tags attached to communities |
| ct_tag_daily | Daily post tag counts per community-date |
| ct_community_daily | Daily fresh post total count per community-date |
| ct_worksheets | Worksheet status per community-date (pending/suggested/confirmed) |
| ct_robots_status | Crawl allowance check results and hash snapshots |
| ct_robots_transitions | Allowance transitions log |
| ct_seen_posts | Fingerprint log (sha256) of processed trend items for dedup |
| ct_memes | Confirmed meme aliases and status (active/retired) |
| ct_meme_candidates | AI-extracted meme candidates and hit counts |
| ct_meme_blacklist | Banned expressions from candidates list |
| ct_meme_daily | Daily confirmed meme frequencies |
...
- `GET,POST,PATCH,DELETE /admin/community-trend/communities`, `PUT /admin/community-trend/communities/:id/meta-tags`
- `GET,POST /admin/community-trend/axes`, `GET,POST,PATCH,DELETE /admin/community-trend/tags`
- `GET /admin/community-trend/worksheets`, `GET /admin/community-trend/worksheets/suggestion`, `POST /admin/community-trend/worksheets/confirm`
- `GET /admin/community-trend/trends/community`, `GET /admin/community-trend/trends/cohort`
- `GET,POST,PATCH,DELETE /admin/community-trend/memes`, `GET,POST,DELETE /admin/community-trend/meme-candidates`
- `GET /admin/community-trend/robots-status` -- robots checks & transition history
...
### Community Trend Pipeline & Tagger
- **1-Pass Tagger**: Tags, meme matches, and new candidates are extracted in a single Gemini call to optimize cost.
- **Freshness-only Counting**: `ct_tag_daily` and `ct_community_daily` only count newly observed posts (carryover from prior day's best board is filtered out by `ct_seen_posts` sha256 fingerprints).
- **Robots Gate**: `allowed=false` forces worksheet to 'manual' mode, skipping auto-crawl.
